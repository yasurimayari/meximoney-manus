...
Durante la comprobación pasiva inicial, una instalación PWA con shell anterior no reconocía aún la ruta nueva de Score. Se rotó el caché del worker a `meximoney-personal-shell-v3` y el cliente registra explícitamente la revisión `build=20260827-v3`; así no reutiliza la respuesta anterior del CDN al actualizar el worker. La comprobación publicada confirmó que el worker versionado ya entrega v3 y, tras limpiar sólo el almacenamiento del navegador de prueba, `/score` muestra el acceso protegido sin 404 ni datos financieros. No se inició sesión, no se concedieron permisos y no se creó ningún registro durante esta validación.

La ruta `/proyectos` también entrega el cliente actualizado. El navegador automático permaneció en el esqueleto de autenticación sin sesión, por lo que no se abrió el contenido protegido ni se simuló una sesión. La revisión de los flujos de creación, edición, archivado y borrado se cubrió mediante tipos y pruebas unitarias; la verificación visual autenticada queda reservada a la propietaria para no crear datos financieros personales de prueba.
