...
function createManualSnapshotText(snapshot: Awaited<ReturnType<typeof getFinanceSnapshot>>, notes: Array<{ title: string; content: string; tag: string; tagColor: string; isPinned: boolean; updatedAt: Date; attachments?: Array<{ fileName: string; fileMimeType: string; fileSizeBytes: number }> }> = [], selectedNoteTag?: string) {
  ...
  return JSON.stringify({
    ...
    personalNotes,
    personalNotesFilter: selectedNoteTag || "all",
  });
}
  ...
  finance: router({
    ...
    workspace: router({
      ...
      transferSave: workspaceFinanceProcedure.input(z.object({
        sourceAccountId: z.number().int().positive(), destinationAccountId: z.number().int().positive(), investmentId: z.number().int().positive().nullable().optional(), amountCents: z.number().int().positive(), occurredAt: z.number().int().positive(), status: z.enum(["confirmed", "estimated", "needs_review"]), bankReference: z.string().trim().max(160).nullable().optional(), notes: z.string().max(3000).nullable().optional(),
      })).mutation(async ({ ctx, input }) => {
        if (ctx.workspaceAccess.role !== "owner" && !ctx.workspaceAccess.canCreateDrafts) throw new TRPCError({ code: "FORBIDDEN", message: "Tu rol no permite crear borradores." });
      ...
      transferUpdate: workspaceFinanceProcedure.input(z.object({
        transferGroupId: z.string().uuid(), sourceAccountId: z.number().int().positive(), destinationAccountId: z.number().int().positive(), amountCents: z.number().int().positive(), occurredAt: z.number().int().positive(), status: z.enum(["confirmed", "estimated", "needs_review"]), bankReference: z.string().trim().max(160).nullable().optional(), notes: z.string().max(3000).nullable().optional(),
      })).mutation(async ({ ctx, input }) => {
        ...
        const suffix = input.notes?.trim() ? ` · ${input.notes.trim()}` : "";
        await db.transaction(async tx => {
          await tx.update(financialTransactions).set({ entityId: source[0].entityId, projectId: source[0].projectId, accountId: source[0].id, type: "transfer_out", scope: source[0].scope, amountCents: input.amountCents, currency: source[0].currency, reportCurrency: source[0].currency, reportAmountCents: input.amountCents, occurredAt, status, reviewStatus, reviewedByUserId: isOwner ? ctx.user.id : null, reviewedAt: isOwner ? new Date() : null, bankReference: input.bankReference?.trim() || null, notes: `Traspaso a ${destination[0].name}${suffix}` }).where(and(eq(financialTransactions.id, outgoing.id), eq(financialTransactions.userId, ctx.workspaceAccess.ownerId)));
          await tx.update(financialTransactions).set({ entityId: destination[0].entityId, projectId: destination[0].projectId, accountId: destination[0].id, type: "transfer_in", scope: destination[0].scope, amountCents: input.amountCents, currency: destination[0].currency, reportCurrency: destination[0].currency, reportAmountCents: input.amountCents, occurredAt, status, reviewStatus, reviewedByUserId: isOwner ? ctx.user.id : null, reviewedAt: isOwner ? new Date() : null, bankReference: input.bankReference?.trim() || null, notes: `Traspaso desde ${source[0].name}${suffix}` }).where(and(eq(financialTransactions.id, incoming.id), eq(financialTransactions.userId, ctx.workspaceAccess.ownerId)));
        });
