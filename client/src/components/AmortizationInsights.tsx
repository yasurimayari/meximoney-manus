  ...
  return <section className="space-y-4" aria-labelledby="amortization-confirmed-summary">
    <div className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-muted/25 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">Basadas únicamente en pagos conciliados.</p>
      </div>
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-3"><Summary label="Capital pagado" value={summary.capitalPaidCents} currency={debt.currency} tone="text-emerald-700"/><Summary label="Intereses pagados" value={summary.totalInterestPaidCents} currency={debt.currency} tone="text-amber-700"/><Summary label="Capital restante" value={summary.capitalRemainingCents} currency={debt.currency} tone="text-primary"/></div>
