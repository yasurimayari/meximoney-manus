...
export function filterWorkspaceSnapshot(snapshot: any, filters: WorkspaceFilters) {
  ...
  const creditCards = (snapshot.creditCards ?? []).filter((item: any) => hasScope(item) && hasCurrency(item));
  return { ...snapshot, transactions, accounts, debts, creditCards, investments: (snapshot.investments ?? []).filter((item: any) => hasScope(item) && hasCurrency(item)), receivables: (snapshot.receivables ?? []).filter((item: any) => hasScope(item) && hasCurrency(item)), fiscalRecords: (snapshot.fiscalRecords ?? []).filter((item: any) => hasScope(item) && hasCurrency(item)), budgets: (snapshot.budgets ?? []).filter(hasScope), goals: (snapshot.goals ?? []).filter(hasScope), documents: (snapshot.documents ?? []).filter(hasScope), calendarEvents: (snapshot.calendarEvents ?? []).filter((item: any) => hasScope(item) && hasCurrency(item) && (!filters.reviewStatus || item.status === filters.reviewStatus)), statements: (snapshot.statements ?? []).filter((item: any) => (isPersonal ? item.scope === "personal" : !entityId || item.entityId === entityId) && (!projectId || item.projectId === projectId) && (!filters.currency || item.filterCurrency === filters.currency)) };
}
