...
export function WorkspaceFilterBar({ snapshot, filters, onChange, showReview = true, statusLabel = "Revisión", statusOptions = [{ value: "approved", label: "Aprobados" }, { value: "pending_review", label: "Pendientes de revisión" }, { value: "draft", label: "Borradores" }] }: { snapshot: any; filters: WorkspaceFilters; onChange: (filters: WorkspaceFilters) => void; showReview?: boolean; statusLabel?: string; statusOptions?: Array<{ value: string; label: string }> }) {
  const currencies = Array.from(new Set([...(snapshot.transactions ?? []).map((item: any) => item.currency), ...(snapshot.accounts ?? []).map((item: any) => item.currency), ...(snapshot.debts ?? []).map((item: any) => item.currency), ...(snapshot.receivables ?? []).map((item: any) => item.currency), ...(snapshot.investments ?? []).flatMap((item: any) => [item.currency, item.reportCurrency])].filter(Boolean))).sort();
...
export function filterWorkspaceSnapshot(snapshot: any, filters: WorkspaceFilters) {
  ...
  return { ...snapshot, transactions, accounts, debts, investments: (snapshot.investments ?? []).filter((item: any) => hasScope(item) && hasCurrency(item)), receivables: (snapshot.receivables ?? []).filter((item: any) => hasScope(item) && hasCurrency(item)), budgets: (snapshot.budgets ?? []).filter(hasScope), goals: (snapshot.goals ?? []).filter(hasScope), documents: (snapshot.documents ?? []).filter(hasScope), calendarEvents: (snapshot.calendarEvents ?? []).filter((item: any) => hasScope(item) && hasCurrency(item) && (!filters.reviewStatus || item.status === filters.reviewStatus)), statements: (snapshot.statements ?? []).filter((item: any) => (!entityId || item.entityId === entityId) && (!projectId || item.projectId === projectId) && (!filters.currency || item.filterCurrency === filters.currency)) };
}
