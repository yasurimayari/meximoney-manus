...
const snapshot = {
  ...
  transactions: [{ id: 1, type: "expense", notes: "Suscripción Notion", accountId: 8, categoryId: 9, contactId: 10, bankReference: "TRX-99881", occurredAt: new Date("2026-08-14T12:00:00Z"), amountCents: 34900, currency: "MXN" }],
...
describe("búsqueda unificada de Registro", () => {
  it("encuentra resultados entre movimientos, inversiones, estados y documentos", () => {
    ...
    expect(findRecordSearchResults(snapshot, { query: "TRX-99881" }).map(item => item.key)).toEqual(["transaction-1"]);
  });
