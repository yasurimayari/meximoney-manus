...
export function exportFinancialPlanPdf({ plan, periods, scenarios, transactions }: FinancialPlanExportInput) {
  ...
  for (let page = 1; page <= pageCount; page += 1) { document.setPage(page); document.setFont("helvetica", "normal"); document.setFontSize(7.2); document.setTextColor(90, 107, 104); document.text(`Meximoney · Página ${page} de ${pageCount} · Revisión manual`, width - 42, height - 24, { align: "right" }); }
