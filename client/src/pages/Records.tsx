      ...
      <header className="page-heading">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            {mode === "document" && <DocumentForm accounts={accounts} debts={data?.debts ?? []} editing={editingDocument} onDone={refresh} />}
          </DialogContent>
        </Dialog>
      </header>
...
function DocumentForm({ accounts, debts, editing, onDone }: { accounts: any[]; debts: any[]; editing: any; onDone: () => void }) {
  ...
  const [form, setForm] = useState(() => ({ name: editing?.name ?? "", type: editing?.type ?? "receipt", documentClass: editing?.documentClass ?? "general", scope: editing?.scope ?? "personal", relatedEntityType: editing?.relatedEntityType ?? "none", relatedEntityId: editing?.relatedEntityId?.toString() ?? "", jurisdiction: editing?.jurisdiction ?? "", referenceUrl: editing?.referenceUrl ?? "", issuedAt: editing?.issuedAt ? new Date(editing.issuedAt).toISOString().slice(0, 10) : "", expiresAt: editing?.expiresAt ? new Date(editing.expiresAt).toISOString().slice(0, 10) : "", reminderAt: editing?.reminderAt ? new Date(editing.reminderAt).toISOString().slice(0, 10) : "", verified: editing?.verified ?? false, notes: editing?.notes ?? "" }));
  const submit = (event: FormEvent) => { event.preventDefault(); mutation.mutate({ id: editing?.id, ...form, relatedEntityId: form.relatedEntityId ? Number(form.relatedEntityId) : null, issuedAt: form.issuedAt ? new Date(`${form.issuedAt}T12:00:00`).getTime() : null, expiresAt: form.expiresAt ? new Date(`${form.expiresAt}T12:00:00`).getTime() : null, reminderAt: form.reminderAt ? new Date(`${form.reminderAt}T12:00:00`).getTime() : null, jurisdiction: form.jurisdiction || null, referenceUrl: form.referenceUrl || null, notes: form.notes || null } as any); };
