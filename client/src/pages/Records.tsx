...
type RecordMode = "movement" | "transfer" | "account" | "category" | "document";
...
export default function Records() {
  ...
  const removeTransfer = trpc.finance.workspace.transferRemove.useMutation({ onSuccess: refresh, onError: error => toast.error(error.message) });
    ...
    <div className="space-y-8">
      <header className="page-heading">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            {mode === "transfer" && <TransferForm accounts={accounts} workspaceAccess={data?.workspaceAccess} onDone={refresh} />}
      ...
      <section className="record-shortcuts" aria-label="Crear registros">
        {[
          ...
          { mode: "transfer" as const, icon: ArrowLeftRight, title: "Traspaso", note: "Entre cuentas propias, no es ingreso" },
...
function TransferForm({ accounts, workspaceAccess, onDone }: { accounts: any[]; workspaceAccess: any; onDone: () => void }) {
  const mutation = trpc.finance.workspace.transferSave.useMutation({ onSuccess: () => { toast.success(workspaceAccess?.role === "owner" ? "Traspaso registrado sin afectar ingresos ni gastos" : "Traspaso enviado a revisión"); onDone(); }, onError: error => toast.error(error.message) });
  const activeAccounts = accounts.filter(account => account.status === "active");
  const [form, setForm] = useState(() => ({ sourceAccountId: "", destinationAccountId: "", amount: "", occurredAt: new Date().toISOString().slice(0, 10), status: "confirmed", notes: "" }));
  const source = activeAccounts.find(account => account.id === Number(form.sourceAccountId));
  const destination = activeAccounts.find(account => account.id === Number(form.destinationAccountId));
  const submit = (event: FormEvent) => { event.preventDefault(); mutation.mutate({ sourceAccountId: Number(form.sourceAccountId), destinationAccountId: Number(form.destinationAccountId), amountCents: toCents(form.amount), occurredAt: new Date(`${form.occurredAt}T12:00:00`).getTime(), status: form.status as any, notes: form.notes || null }); };
  return <form className="form-grid" onSubmit={submit}><div className="form-field span-2"><div className="rounded-xl border border-primary/15 bg-primary/5 p-3 text-sm text-muted-foreground">Registra las dos partes del movimiento: salida y entrada. El traspaso no es un ingreso ni un gasto, y Meximoney no mueve dinero real.</div></div><div className="form-field"><Label>Cuenta de origen</Label><select required value={form.sourceAccountId} onChange={event => setForm({ ...form, sourceAccountId: event.target.value })}><option value="">Elige la cuenta de salida</option>{activeAccounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.currency}</option>)}</select></div><div className="form-field"><Label>Cuenta de destino</Label><select required value={form.destinationAccountId} onChange={event => setForm({ ...form, destinationAccountId: event.target.value })}><option value="">Elige la cuenta de entrada</option>{activeAccounts.map(account => <option key={account.id} value={account.id} disabled={account.id === Number(form.sourceAccountId)}>{account.name} · {account.currency}</option>)}</select></div>{source && destination && source.currency !== destination.currency ? <p className="form-field span-2 text-sm text-destructive">Estas cuentas usan monedas distintas. Registra una conversión manual confirmada por separado si aplica.</p> : null}<div className="form-field"><Label>Importe{source ? ` (${source.currency})` : ""}</Label><Input type="number" min="0.01" step="0.01" required value={form.amount} onChange={event => setForm({ ...form, amount: event.target.value })} /></div><div className="form-field"><Label>Fecha</Label><Input type="date" required value={form.occurredAt} onChange={event => setForm({ ...form, occurredAt: event.target.value })} /></div><div className="form-field"><Label>Calidad del dato</Label><select value={form.status} onChange={event => setForm({ ...form, status: event.target.value })}><option value="confirmed">Confirmado</option><option value="estimated">Estimado</option><option value="needs_review">Pendiente de revisar</option></select></div><div className="form-field span-2"><Label>Nota opcional</Label><Textarea placeholder="Ej. Fondeo de Plata desde Santander" value={form.notes} onChange={event => setForm({ ...form, notes: event.target.value })} /></div><div className="form-actions span-2"><Button type="submit" disabled={mutation.isPending || Boolean(source && destination && source.currency !== destination.currency) || activeAccounts.length < 2}>{mutation.isPending ? "Guardando…" : workspaceAccess?.role === "owner" ? "Registrar traspaso" : "Enviar a revisión"}</Button></div></form>;
}
