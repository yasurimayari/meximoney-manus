...
function MetricCard({ icon: Icon, label, value, note, tone = "neutral" }: { icon: typeof WalletCards; label: string; value: ReactNode; note: string; tone?: "neutral" | "positive" | "warm" | "danger" }) {
...
export default function Home() {
  ...
  return <div className="space-y-8">
    ...
    <section><div className="section-heading"><div><p className="eyebrow">Resumen del periodo · {currency}</p><h2>Tu situación actual</h2></div><span className="period-chip">{formatDate(data.dashboard.periodStart, { month: "long", year: "numeric" })}</span></div><div className="metric-grid"><MetricCard icon={CircleDollarSign} label="Flujo neto" value={<MoneyText cents={data.dashboard.cashFlow.netCashFlowCents} currency={currency} />} note={`$<MoneyText cents={data.dashboard.cashFlow.incomeCents} currency={currency} /> ingresos · $<MoneyText cents={data.dashboard.cashFlow.expenseCents} currency={currency} /> gastos`} tone={data.dashboard.cashFlow.netCashFlowCents >= 0 ? "positive" : "danger"} /><MetricCard icon={Landmark} label="Patrimonio neto" value={<MoneyText cents={data.dashboard.netWorth.netWorthCents} currency={currency} />} note={`$<MoneyText cents={data.dashboard.netWorth.assetCents} currency={currency} /> activos · $<MoneyText cents={data.dashboard.netWorth.liabilityCents} currency={currency} /> deudas`} /><MetricCard icon={WalletCards} label="Liquidez disponible" value={<MoneyText cents={data.dashboard.liquidity.liquidCents} currency={currency} />} note={data.dashboard.liquidity.coverageMonths === null ? "Registra gastos esenciales para estimar cobertura" : `${data.dashboard.liquidity.coverageMonths.toFixed(1)} meses de cobertura estimada`} tone="warm" /><MetricCard icon={CircleDollarSign} label="Deuda activa" value={<MoneyText cents={debtBalance} currency={currency} />} note={pendingDebtCurrencyCount ? `${reportCurrencyDebts.length} en ${currency} · ${pendingDebtCurrencyCount} pendiente de valorar` : `${activeDebts.length} obligaciones activas o por revisar`} tone={activeDebts.length ? "danger" : "positive"} /></div></section>

    <div className="dashboard-grid">
      <section className="content-card main-card"><div className="card-title-row"><div><h2>Flujo manual del mes</h2><p>Los ingresos y gastos excluyen transferencias internas y usan sólo conversión confirmada a {currency}.</p></div><Link className="text-link" href="/movimientos">Ver movimientos</Link></div><div className="flow-visual"><div className="flow-row"><span><i className="flow-dot dot-income"><ArrowDownLeft className="size-3" /></i>Ingresos</span><strong><MoneyText cents={data.dashboard.cashFlow.incomeCents} currency={currency} /></strong></div><div className="flow-bar"><span className="income-bar" style={{ width: `${data.dashboard.cashFlow.incomeCents > 0 ? 100 : 0}%` }} /></div><div className="flow-row"><span><i className="flow-dot dot-expense"><ArrowUpRight className="size-3" /></i>Gastos</span><strong><MoneyText cents={data.dashboard.cashFlow.expenseCents} currency={currency} /></strong></div><div className="flow-bar"><span className="expense-bar" style={{ width: `${data.dashboard.cashFlow.incomeCents ? Math.min(100, (data.dashboard.cashFlow.expenseCents / data.dashboard.cashFlow.incomeCents) * 100) : 0}%` }} /></div><div className="flow-footer"><span>Resultado del periodo</span><strong className={data.dashboard.cashFlow.netCashFlowCents >= 0 ? "amount-positive" : "amount-negative"}><MoneyText cents={data.dashboard.cashFlow.netCashFlowCents} currency={currency} /></strong></div>{data.dashboard.cashFlow.pendingConversionCount > 0 ? <div className="mt-4 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-900"><AlertTriangle className="mr-1 inline size-3.5" />{data.dashboard.cashFlow.pendingConversionCount} {data.dashboard.cashFlow.pendingConversionCount === 1 ? "partida quedó" : "partidas quedaron"} fuera del total hasta confirmar su conversión manual a {currency}.</div> : null}</div></section>
