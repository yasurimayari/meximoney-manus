...
export default function Home() {
  ...
  return <div className="space-y-8">
    ...
    {closingPending ? <section className="flex flex-col gap-3 rounded-2xl border border-amber-300 bg-amber-50 px-5 py-4 text-amber-950 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="mt-0.5 rounded-xl bg-amber-200 p-2 text-amber-800"><AlertTriangle className="size-4" /></span><div><p className="font-semibold">Cierre mensual pendiente</p><p className="mt-1 text-sm">Aún no hay un cierre guardado para {formatDate(data.dashboard.periodStart, { month: "long", year: "numeric" })}. Confirma activos, pasivos y movimientos antes de guardar la fotografía histórica.</p></div></div><Link href="/estados"><Button variant="outline" className="shrink-0 border-amber-400 bg-white text-amber-950 hover:bg-amber-100">Preparar cierre</Button></Link></section> : null}
