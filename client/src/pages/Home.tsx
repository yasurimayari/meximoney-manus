...
export default function Home() {
  ...
  return <div className="space-y-8">
    ...
    <div className="dashboard-grid">
      ...
      <section className="content-card"><div className="card-title-row"><div><h2>Calidad de tus datos</h2><p>Revisamos coherencia, no tus cuentas externas.</p></div><Link href="/calidad" className="text-link">Revisar</Link></div>{data.dashboard.qualityIssues.length === 0 ? <div className="quality-good"><CheckCircle2 className="size-5" /><div><strong>Todo listo para analizar</strong><p>No hay alertas de calidad en tus datos manuales.</p></div></div> : <div className="quality-list">{data.dashboard.qualityIssues.slice(0, 4).map((issue, index) => <div className={`quality-item severity-${issue.severity}`} key={`${issue.code}-${index}`}><AlertTriangle className="size-4" /><span>{issue.label}</span></div>)}</div>}</section>
    </div>
