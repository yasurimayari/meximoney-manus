...
import { clearOfflineVault, getOfflineVaultStatus, saveOfflineSnapshot, type OfflineSnapshotSummary, type OfflineVaultStatus } from "@/lib/offlineVault";
import { trpc } from "@/lib/trpc";
import { CloudDownload, LockKeyhole, RefreshCw, ShieldCheck, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

const policyVersion = "2026-09-offline-vault-v2";

export default function OfflineData() {
  const { user } = useAuth();
  const { data, isLoading, isError, error } = trpc.finance.offlineSnapshot.useQuery(dashboardPeriodQuery, { enabled: Boolean(user) });
  ...
  const [status, setStatus] = useState<OfflineVaultStatus | null>(null);
  const [summary, setSummary] = useState<OfflineSnapshotSummary | null>(null);
  const [saving, setSaving] = useState(false);

  const refreshStatus = () => getOfflineVaultStatus().then(setStatus).catch(() => setStatus(null));
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const saved = await saveOfflineSnapshot(data, pin, user.id);
      await consent.mutateAsync({ accepted: true, policyVersion });
      setPin(""); setConfirmation(""); setSummary(saved); setStatus({ ownerUserId: user.id, cachedAt: saved.cachedAt });
      toast.success("Copia offline cifrada y vinculada a tu cuenta en este dispositivo.");
    } catch (error) {
  ...
  if (isLoading || !data) {
    if (isError) return <div className="page-loading"><p className="text-sm font-semibold">La bóveda offline es sólo para la propietaria del espacio.</p><p className="mt-2 text-sm text-muted-foreground">{error?.message ?? "No tienes permiso para crear una copia offline de este espacio."}</p></div>;
    return <div className="page-loading">Preparando la bóveda offline personal…</div>;
  }
  const displayed = summary ?? { cachedAt: status?.cachedAt ?? "", accounts: 0, creditCards: 0, debts: 0, transactions: 0, budgets: 0, investments: 0, calendarEvents: 0, tasks: 0, travelPlans: 0 };
  return <div className="space-y-7"><header className="page-heading"><div><p className="eyebrow">PWA personal</p><h1>Tus datos, disponibles con control.</h1><p>Guarda una copia cifrada de tu información financiera en este dispositivo para consultarla cuando no haya Internet.</p></div></header><section className="content-card"><div className="card-title-row"><div><h2>Bóveda offline cifrada</h2><p>Incluye cuentas, tarjetas, deudas, movimientos, presupuestos, inversiones, objetivos, tareas, viajes, calendario, estados y metadatos documentales. Excluye contraseñas, sesiones, correo, fecha de nacimiento, foto, archivos y enlaces de documentos.</p></div><span className="workspace-role"><LockKeyhole className="size-4" /> Sólo propietaria</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><OfflineStat label="Movimientos" value={summary ? displayed.transactions : "—"} /><OfflineStat label="Cuentas y tarjetas" value={summary ? displayed.accounts + displayed.creditCards : "—"} /><OfflineStat label="Tareas" value={summary ? displayed.tasks : "—"} /><OfflineStat label="Viajes" value={summary ? displayed.travelPlans : "—"} /></div>{status ? <div className="mt-5 rounded-xl border border-primary/15 bg-primary/[0.035] p-4 text-sm"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 size-5 shrink-0 text-primary" /><div><p className="font-semibold">Copia local cifrada disponible</p><p className="mt-1 text-muted-foreground">Última actualización: {new Date(status.cachedAt).toLocaleString("es-MX")}. Los contadores se muestran de nuevo al renovar la copia; no se guardan en claro en IndexedDB.</p></div></div></div> : null}<div className="mt-6 grid gap-4 rounded-2xl border bg-muted/20 p-5 md:grid-cols-2"><div className="form-field"><Label htmlFor="offline-pin">Código local de desbloqueo</Label><Input id="offline-pin" type="password" autoComplete="new-password" minLength={8} value={pin} onChange={event => setPin(event.target.value)} placeholder="Al menos 8 caracteres" /><small>No se envía al servidor ni se guarda. Si lo olvidas, elimina la copia y crea una nueva al estar en línea.</small></div><div className="form-field"><Label htmlFor="offline-pin-confirm">Repite el código local</Label><Input id="offline-pin-confirm" type="password" autoComplete="new-password" minLength={8} value={confirmation} onChange={event => setConfirmation(event.target.value)} placeholder="Repite el código" /></div><label className="md:col-span-2 flex items-start gap-3 rounded-xl bg-background p-4 text-xs leading-5"><input className="mt-0.5 size-4 accent-primary" type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} /><span>Entiendo que esta copia contiene mis datos financieros cifrados en este dispositivo personal. Sé que debo proteger mi equipo y que puedo eliminar la copia desde esta pantalla o mediante el borrado completo de datos.</span></label><div className="md:col-span-2 flex flex-wrap gap-3"><Button type="button" disabled={saving || consent.isPending} onClick={save}><CloudDownload className="size-4" /> {saving ? "Protegiendo copia…" : status ? "Actualizar copia cifrada" : "Guardar copia cifrada"}</Button>{status ? <Button type="button" variant="outline" disabled={saving || consent.isPending} onClick={remove}><Trash2 className="size-4" /> Eliminar copia de este dispositivo</Button> : null}</div></div></section><section className="content-card"><div className="card-title-row"><div><h2>Cómo funciona sin conexión</h2><p>La instalación abre una vista local bloqueada con tu código. Puedes buscar y consultar la copia cifrada, pero cualquier alta, edición, exportación, pago, inversión o transferencia seguirá requiriendo conexión y una acción tuya.</p></div><RefreshCw className="size-5 text-primary" /></div></section></div>;
}

function OfflineStat({ label, value }: { label: string; value: number | string }) {
