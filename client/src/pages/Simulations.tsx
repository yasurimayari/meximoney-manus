...
function MetricCard({ label, value, attention = false }: { label: string; value: string; attention?: boolean }) {
...
function AllocationCard({ icon: Icon, label, value, currency }: { icon: typeof Landmark; label: string; value: number; currency: string }) {
  return <article className="rounded-xl border bg-muted/30 p-4"><Icon className="size-4 text-primary"/><p className="mt-3 text-sm text-muted-foreground">{label}</p><strong className="mt-1 block text-xl">{formatMoney(value, currency)}</strong></article>;
}
...
export default function Simulations() {
  ...
  const budgetResults = calculateBudgetVsActual(scopedData, currency).filter(result => sameMonth(result.budget.periodStart, periodStart));
  ...
  const hasDebtModelGap = debtResult.invalidDebtNames.length > 0 || debtResult.nonAmortizingDebtNames.length > 0;

  const updateDebtInput = (id: string, field: "rate" | "minimum", value: string, debt: SimulationDebt) => {
  ...
  return <div className="space-y-7">
    ...
    {tab === "debts" ? <section className="space-y-4"><SimulationBoundary/><section className="content-card"><div className="flex flex-col gap-4 border-b pb-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-lg font-semibold">Avalancha y bola de nieve</h2><p className="mt-1 text-sm text-muted-foreground">Se conserva el pago mínimo de cada obligación y el pago extra temporal se dirige a la prioridad del método elegido.</p></div><div className="flex flex-wrap gap-3"><div className="form-field min-w-44"><Label>Pago extra mensual</Label><Input type="number" min="0" step="0.01" value={extraPayment} onChange={event => setExtraPayment(event.target.value)} /></div><div className="form-field min-w-44"><Label>Método</Label><select value={strategy} onChange={event => setStrategy(event.target.value as DebtStrategy)}><option value="avalanche">Avalancha · mayor tasa</option><option value="snowball">Bola de nieve · menor saldo</option></select></div></div></div>
      {sourceDebts.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">No hay deudas o tarjetas activas comparables en {currency} con los filtros elegidos.</p> : <><div className="mt-5 grid gap-3 md:grid-cols-3"><MetricCard label="Horizonte modelado" value={hasDebtModelGap ? "Datos por completar" : debtResult.cappedAtMaximum ? "600+ meses" : `${debtResult.months} meses`} attention={hasDebtModelGap}/><MetricCard label="Interés proyectado" value={hasDebtModelGap ? "—" : formatMoney(debtResult.totalInterestCents, currency)} attention={hasDebtModelGap}/><MetricCard label="Saldo no liquidado" value={hasDebtModelGap ? "—" : formatMoney(debtResult.remainingBalanceCents, currency)} attention={hasDebtModelGap}/></div>
        <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="border-b text-left text-muted-foreground"><tr><th className="p-3">Obligación</th><th className="p-3">Origen</th><th className="p-3 text-right">Saldo</th><th className="p-3">Tasa anual usada</th><th className="p-3">Pago mínimo usado</th></tr></thead><tbody>{sourceDebts.map(debt => { const input = debtInputs[debt.id]; return <tr className="border-b" key={debt.id}><td className="p-3 font-medium">{debt.name}</td><td className="p-3">{debt.source === "credit_card" ? "Tarjeta" : "Deuda"}</td><td className="p-3 text-right">{formatMoney(debt.balanceCents, currency)}</td><td className="p-3"><Input className="h-8 w-28" type="number" min="0" step="0.01" placeholder="Pendiente" value={input?.rate ?? (debt.interestRateBps === null ? "" : (debt.interestRateBps / 100).toString())} onChange={event => updateDebtInput(debt.id, "rate", event.target.value, debt)} /></td><td className="p-3"><Input className="h-8 w-28" type="number" min="0" step="0.01" value={input?.minimum ?? fromCents(debt.minimumPaymentCents)} onChange={event => updateDebtInput(debt.id, "minimum", event.target.value, debt)} /></td></tr>; })}</tbody></table></div>
      </>}
