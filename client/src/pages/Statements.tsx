...
export default function Statements() {
  ...
  const statementInput = useMemo(() => ({ periodStart: new Date(`${period}-01T12:00:00`).getTime(), scope }), [period, scope]);
  ...
  return <div className="space-y-7">
    ...
    <form className="content-card statement-controls" onSubmit={submit}><div className="form-field"><Label>Periodo</Label><Input type="month" value={period} onChange={event => setPeriod(event.target.value)} /></div><div className="form-field"><Label>Ámbito</Label><select value={scope} onChange={event => setScope(event.target.value as any)}><option value="personal">Personal</option><option value="business">Empresarial</option><option value="mixed">Consolidado manual</option></select></div><div className="form-field"><Label>Estado del cierre</Label><select value={status} onChange={event => setStatus(event.target.value as any)}><option value="draft">Borrador</option><option value="closed">Cerrado</option></select></div><div className="form-field statement-note"><Label>Notas del cierre</Label><Textarea placeholder="Alcance, conciliaciones pendientes o criterios manuales" value={notes} onChange={event => setNotes(event.target.value)} /></div><Button type="submit" className="btn-primary" disabled={save.isPending}><Save className="size-4" /> {save.isPending ? "Guardando…" : "Guardar estado"}</Button></form>
    ...
    <section className="content-card"><div className="card-title-row"><div><p className="eyebrow">Archivo privado</p><h2>Cierres guardados</h2></div><span className="text-sm text-muted-foreground">{savedStatements.length} estados</span></div>{savedStatements.length === 0 ? <p className="mt-4 text-sm text-muted-foreground">Aún no has guardado un cierre. El cálculo visible se actualiza con los registros manuales actuales.</p> : <div className="statement-history">{savedStatements.map((item: any) => <article key={item.id}><div><span className={`scope-pill scope-${item.scope}`}>{scopeLabel[item.scope]}</span><strong className="capitalize">{formatDate(item.periodStart, { month: "long", year: "numeric" })}</strong><small>{item.status === "closed" ? "Cerrado" : "Borrador"}{item.notes ? ` · ${item.notes}` : ""}</small></div><div><span>Resultado</span><strong className={item.netCashFlowCents >= 0 ? "amount-positive" : "amount-negative"}>{formatMoney(item.netCashFlowCents, currency)}</strong></div><div><span>Patrimonio</span><strong>{formatMoney(item.netWorthCents, currency)}</strong></div></article>)}</div>}</section>
  </div>;
}
