...
await call("finance.budgets.save", token, { entityId: workspace.entities[0].id, projectId: null, categoryId: null, scope: "personal", periodStart: new Date(new Date().getFullYear(), new Date().getMonth(), 1).getTime(), plannedCents: 10000, type: "expense", notes: "Presupuesto QA manual" });
await call("finance.profile.save", token, { currency: "MXN", householdSize: 1, dependents: 0, minimumLiquidityCents: 0, referenceEssentialExpensesCents: 0, futureTaxReserveCents: 25000, futureTaxDueAt: Date.now() + 24 * 60 * 60 * 1000, personalProfileConsent: false, notes: "Reserva QA manual" });
...
await call("finance.notifications.savePreferences", secondToken, { inAppEnabled: false, calendarEnabled: false, documentsEnabled: false, debtsEnabled: false, reviewsEnabled: false, budgetEnabled: false, taxReserveEnabled: false });
...
await call("finance.notifications.savePreferences", token, { inAppEnabled: true, calendarEnabled: true, documentsEnabled: false, debtsEnabled: false, reviewsEnabled: true, budgetEnabled: false, taxReserveEnabled: false });
...
const report = { consentRequired, defaultsLoaded: first.preferences.inAppEnabled === true, calendarCreated: Boolean(calendarNotification), manualRules: first.notifications.some(item => item.type === "budget") && first.notifications.some(item => item.type === "tax_reserve"), ownerIsolation: isolated.preferences.inAppEnabled && isolated.notifications.some(item => item.id === calendarNotification.id && !item.readAt), preferencesSaved: read.preferences.documentsEnabled === false && read.preferences.debtsEnabled === false && read.preferences.budgetEnabled === false && read.preferences.taxReserveEnabled === false, markRead: read.notifications.some(item => item.id === calendarNotification.id && item.readAt), dismiss: !dismissed.notifications.some(item => item.id === calendarNotification.id), globalSilence: silenced.notifications.length === 0 };
